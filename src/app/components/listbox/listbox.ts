...
@Component({
    ...
    template: `
        <div
            [ngClass]="{
                'ui-listbox ui-inputtext ui-widget ui-widget-content ui-corner-all': true,
                ...
                <ul class="ui-listbox-list">
                    <li
                        *ngFor="let option of options; let i = index"
                        [style.display]="
                            isItemVisible(option) ? 'block' : 'none'
                        "
                        [attr.tabindex]="option.disabled ? null : '0'"
                        [ngClass]="{
                            'ui-listbox-item ui-corner-all': true,
                            'ui-state-highlight': isSelected(option),
...
export class Listbox implements AfterContentInit, ControlValueAccessor {
    ...
    removeOption(option: any): void {
        this.value = this.value.filter(
            val => !ObjectUtils.equals(val, option.value, this.dataKey)
        );
    }
    ...
    get allChecked(): boolean {
        if (this.filterValue) return this.allFilteredSelected();
            ...
            return (
                ...
                this.options &&
                this.value.length > 0 &&
                    this.value.length === this.getEnabledOptionCount()
            );
    }
    ...
    isItemVisible(option: SelectItem): boolean {
        if (this.filterValue) {
            let visible;
            let filterText = ObjectUtils.removeAccents(
                this.filterValue
            ).toLowerCase();
            ...
            switch (this.filterMode) {
                case "startsWith":
                    visible =
                        ObjectUtils.removeAccents(option.label)
                            .toLowerCase()
    ...
    findNextItem(item) {
        ...
        if (nextItem)
            return DomHandler.hasClass(nextItem, "ui-state-disabled") ||
    ...
    findPrevItem(item) {
        ...
        if (prevItem)
            return DomHandler.hasClass(prevItem, "ui-state-disabled") ||
